                        ...
                        <div class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" id="campaign-name" value="Security Alert Campaign"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" id="subject" value="Urgent: Security Alert for {{target}}"
                                           class="w-full p-4 bg-gray-900 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-2">Action Link</label>
                                    <input type="text" id="phish-link" placeholder="https://verify.example.com"
                                           class="w-full p-4 bg-gray-900 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500">
                                </div>
                            </div>
    ...
    <div style="background: #1a202c; color: #a0aec0; padding: 20px; text-align: center; font-size: 12px;">
        <p>© 2024 MagicPro Security System. All rights reserved.</p>
                                ...
                                <div class="text-xs text-gray-500 mt-2">
                                    Available placeholders: {{target}}, {{email}}, {{id}}, {{sender}}, {{phish}}, {{date}}, {{time}}
                                </div>
                            </div>
                            ...
                            <div class="flex space-x-4">
                                <button onclick="launchCampaign()" id="launch-btn"
                                    ...
                                    <i class="fas fa-play mr-2"></i> Launch Campaign
                                </button>
                                <button onclick="saveTemplate()" id="save-btn"
                                        class="p-4 bg-gray-700 rounded-xl hover:bg-gray-600 transition" title="Save Template">
                                    <i class="fas fa-save"></i>
                                </button>
                                <button onclick="loadDefaultTemplate()"
                                        class="p-4 bg-gray-700 rounded-xl hover:bg-gray-600 transition" title="Load Default">
                ...
                <div>
                    <div class="bg-gray-800 rounded-2xl p-6 h-full">
                        ...
                        <div class="terminal rounded-xl p-4 h-96 overflow-y-auto" id="terminal">
                            <div class="text-green-400">$ Initializing MagicPro v3.0...</div>
                            <div class="text-green-400">$ Quantum SMTP Matrix: ONLINE</div>
                            <div class="text-green-400">$ Ready for campaign deployment</div>
                        </div>
                        ...
                        <div class="mt-6 space-y-3">
                            ...
                            <div class="flex justify-between">
                                <span class="text-gray-400">Current Batch:</span>
                                <span id="current-batch" class="font-mono text-sm">0/0</span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span id="eta" class="font-mono text-sm">--:--</span>
                            </div>
                        </div>

                        <!-- Quick Actions -->
                        <div class="mt-6 grid grid-cols-2 gap-3">
                            <button onclick="clearTerminal()" class="p-2 bg-gray-700 rounded-lg hover:bg-gray-600 text-sm">
                                <i class="fas fa-broom mr-2"></i> Clear
                            </button>
                            <button onclick="testConnection()" class="p-2 bg-gray-700 rounded-lg hover:bg-gray-600 text-sm">
                                <i class="fas fa-wifi mr-2"></i> Test
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gray-800 rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="template-category" class="w-full p-3 bg-gray-900 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
    ...
    <script>
        ...
        function setupSocketListeners() {
            ...
            socket.on('disconnect', () => {
                logToTerminal('Disconnected from server', 'warning');
            });
        }
        ...
        function updateProgressDisplay(data) {
            ...
            document.getElementById('current-batch').textContent = `${data.currentBatch}/${data.totalBatches}`;
        ...
        function renderTemplates(templates) {
            ...
            templatesList.innerHTML = templates.map(template => `
                <div class="bg-gray-900 rounded-xl p-4 hover:bg-gray-800 cursor-pointer transition transform hover:-translate-y-1">
                    <div class="flex justify-between items-center mb-2">
                        <h3 class="font-bold truncate">${template.name}</h3>
                        <span class="text-xs px-2 py-1 bg-gray-800 rounded">${template.category}</span>
                    </div>
        ...
        function loadDefaultTemplate() {
            ...
            logToTerminal('Loaded default template', 'info');
        }
        ...
        function addSampleTargets() {
            const samples = Array.from({length: 50}, (_, i) => 
                `user${i + 1}@example${Math.floor(Math.random() * 5) + 1}.com`
            ).join('\\n');
            ...
            logToTerminal('Added 50 sample targets', 'info');
        }

        function clearTerminal() {
            document.getElementById('terminal').innerHTML = '<div class="text-green-400">$ Terminal cleared</div>';
        }
        ...
        async function testConnection() {
            try {
                ...
                if (response.ok) {
                    logToTerminal('Server connection: OK', 'success');
                }
            } catch (error) {
                logToTerminal('Server connection: FAILED', 'error');
            }
        }
    </script>
</body>
